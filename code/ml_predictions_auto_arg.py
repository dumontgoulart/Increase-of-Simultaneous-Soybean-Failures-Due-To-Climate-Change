# -*- coding: utf-8 -*-
"""
Created on Thu Feb  3 10:31:15 2022
...
# full_model_hyb = joblib.load('sklearn_pipeline_arg_ANN_67.pkl')

# # Then, load the Keras model:
# full_model_hyb['estimator'].model = load_model('hybrid_arg_ANN_67.h5')

# # Data for testing: 
# X = pd.read_csv('dataset_input_hybrid_forML_arg.csv', index_col=[0,1,2],)
# y = pd.read_csv('dataset_obs_yield_forML_arg.csv', index_col=[0,1,2],)

# X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=0)

# y_pred = full_model_hyb.predict(X_test)
...
### Climatic variables - Extreme weather    
    ...
    # Selected features
    list_features_arg = ['prcptot', 'r10mm', 'txm'  ]# 'dtr', 'tnm', 'txge35', 'tr', 'txm', 'tmm', 'tnn'
    DS_clim_ext_projections = DS_clim_ext_projections[list_features_arg]
    DS_clim_ext_projections = DS_clim_ext_projections.where(DS_y_epic_arg_det.mean('time') >= -5.0)
    ...
    DS_clim_ext_projections_arg = DS_clim_ext_projections_combined.where( DS_chosen_calendar_arg >= 0 )
    
    DS_clim_ext_projections_arg_det = DS_clim_ext_projections_arg #detrend_dataset(DS_clim_ext_projections_arg, mean_data = DS_exclim_arg_det_clip )
    DS_clim_ext_projections_arg['prcptot'].mean(['lat','lon']).plot()
    DS_clim_ext_projections_arg_det['prcptot'].mean(['lat','lon']).plot()
    plt.show()
    
    DS_clim_ext_projections_arg['txm'].mean(['lat','lon']).plot()
    DS_clim_ext_projections_arg_det['txm'].mean(['lat','lon']).plot()
    ...
    for feature in list(DS_clim_ext_projections_arg_det.keys()):
        ### Reshape and shift for 24 months for every year.
        df_clim_proj_shift = reshape_shift(DS_clim_ext_projections_arg_det[feature])
        df_clim_proj_shift_12 = reshape_shift(DS_clim_ext_projections_arg_det[feature], shift_time = 12)
        ...
        df_clim_proj_shift = (df_clim_proj_shift_twoyears.dropna().join(df_calendar_month_arg)
    ...
    DS_feature_proj_6mon_arg = xr.Dataset.from_dataframe(df_feature_proj_6mon)
    DS_feature_proj_6mon_arg_det = detrend_dataset(DS_feature_proj_6mon_arg, deg = 'free', mean_data = DS_feature_proj_6mon_arg ) # DS_feature_proj_6mon_arg / DS_feature_season_6mon_arg_det
    df_feature_proj_6mon_arg_det = DS_feature_proj_6mon_arg_det.to_dataframe().dropna()

    for feature in df_feature_season_6mon_arg.columns:
        df_feature_proj_6mon[feature].groupby('time').mean().plot(label = 'old')
        df_feature_proj_6mon_arg_det[feature].groupby('time').mean().plot(label = 'detrend')
        plt.title(f'{feature}')
        plt.legend()
        plt.show()

    for feature in ['prcptot_2', 'prcptot_3', 'prcptot_4']:
        df_feature_proj_6mon_arg_det[feature][df_feature_proj_6mon_arg_det[feature] < 0] = 0

    # UPDATE DETRENDED VALUES
    df_feature_proj_6mon = df_feature_proj_6mon_arg_det
    ...
    def epic_projections_function_co2(co2_scenario):
        ...
        DS_y_epic_proj_arg = DS_y_epic_proj.where(DS_y_epic_arg_det.mean('time') >= -5.0 )
        DS_y_epic_proj_arg = DS_y_epic_proj_arg.where(DS_clim_ext_projections['prcptot'].mean('time') >= -100.0 )
        

        DS_y_epic_proj_arg_det = xr.DataArray( detrend_dim(DS_y_epic_proj_arg['yield-soy-noirr'], 'time') + DS_y_epic_arg_det.mean('time'), name= DS_y_epic_proj_arg['yield-soy-noirr'].name, attrs = DS_y_epic_proj_arg['yield-soy-noirr'].attrs)
        
        # plot_2d_map(DS_y_epic_proj_arg_det.mean('time'))
        
        DS_y_epic_proj_arg['yield-soy-noirr'].mean(['lat', 'lon']).plot()
        DS_y_epic_proj_arg_det.mean(['lat', 'lon']).plot()
        plt.title('Detrend'+ "_"+ model + "_"+ rcp_scenario+"_"+co2_scenario)
        plt.show()
        
        df_y_epic_proj_arg = DS_y_epic_proj_arg_det.to_dataframe().dropna()
        df_y_epic_proj_arg = df_y_epic_proj_arg.reorder_levels(['time','lat','lon']).sort_index()
        
        #%% HYBRID
        
        df_hybrid_proj_2 = pd.concat([df_y_epic_proj_arg, df_feature_proj_6mon], axis = 1 )
        df_hybrid_proj_test_2 = df_hybrid_proj_2.query(f"time>=2016 and time <= {end_year}")

        # Predicting hybrid results
        df_prediction_proj_2 = df_y_epic_proj_arg.query(f"time>=2016 and time <= {end_year}").copy() 
        ...
        plt.plot(DS_y_epic_proj_arg_det.time.values, DS_y_epic_proj_arg_det.mean(['lat','lon']), label = 'EPIC pure')
        ...
        plt.plot(DS_y_epic_proj_arg_det.time.values, DS_y_epic_proj_arg_det.mean(['lat','lon'])/DS_y_epic_proj_arg_det.mean(), label = 'EPIC pure')
        plt.title('Hybrid'+ "_"+ model + "_"+rcp_scenario+"_"+co2_scenario)
        ...
        #%% EPIC SIM
        df_predic_epic_test = df_y_epic_proj_arg.copy()
        df_predic_epic_test.loc[:,'yield-soy-noirr'] = full_model_epic_arg.predict(df_y_epic_proj_arg)
        ...
        # CLIMATIC model
        df_pred_clim_proj_test = df_y_epic_proj_arg.copy()
        df_pred_clim_proj_test.loc[:,'yield-soy-noirr'] = full_model_exclim_dyn_arg.predict(df_feature_proj_6mon)
        df_pred_clim_proj_test.index = df_feature_proj_6mon.index
        ...
        # WEIGHTING SCHEME
        # DS_harvest_area_globiom = xr.open_dataset('../../paper_hybrid_agri/data/soy_arg_harvest_area_05x05.nc')
        # DS_harvest_area_globiom['harvest_area'] = DS_harvest_area_globiom['harvest_area'].where(DS_hybrid_proj_2['yield-soy-noirr'].mean('time')>=-2)
        # plot_2d_map(DS_harvest_area_globiom['harvest_area'])
        
        # total_area = DS_harvest_area_globiom['harvest_area'].sum()
        
        DS_harvest_area_globiom =  xr.open_dataset("../../paper_hybrid_agri/data/americas_mask_ha.nc", decode_times=False).sel(longitude=slice(-78.25,-34)).rename({'latitude': 'lat', 'longitude': 'lon'}) # xr.open_dataset('../../paper_hybrid_agri/data/soy_harvest_area_globiom_05x05_2b.nc').mean('time')
        ...
        DS_epic_pure_weighted = ((DS_y_epic_proj_arg_det * DS_harvest_area_globiom['harvest_area'] ) / total_area).to_dataset(name = 'yield-soy-noirr')
        ...
        plt.plot(DS_y_epic_proj_arg_det.time.values, DS_y_epic_proj_arg_det.mean(['lat','lon']), label = 'Pure EPIC')
        ...
        plot_2d_map(DS_y_epic_proj_arg_det.sel(time=weighted_timeseries_min)/DS_y_epic_proj_arg_det.mean('time'))
        plot_2d_map(DS_hybrid_proj_2['yield-soy-noirr'].sel(time=weighted_timeseries_min)/DS_hybrid_proj_2["yield-soy-noirr"].mean('time'))
        plot_2d_map(DS_hybrid_proj_2['yield-soy-noirr'].sel(time=weighted_timeseries_min)/DS_hybrid_proj_2["yield-soy-noirr"].mean('time') - DS_y_epic_proj_arg_det.sel(time=weighted_timeseries_min)/DS_y_epic_proj_arg_det.mean('time'))
    ...
    #%% GLOBIOM SHIFTERS
        ...
        df_hybrid_proj_test_2 = df_hybrid_proj_test_2.rename(columns = {'yield-soy-noirr':f'yield-soy-noirr_{co2_scenario}'})
        df_prediction_proj_2 = df_prediction_proj_2.rename(columns = {'yield-soy-noirr':f'yield-soy-noirr_{co2_scenario}'})
...
# UKESM model
df_hybrid_proj_ukesm_585_arg, df_prediction_proj_ukesm_585_arg = projections_generation_hybrid(model = 'ukesm', rcp_scenario = 'ssp585', region = "_arg", hybrid_model_full = full_model_hyb_arg, start_date='01-01-2015', end_date='31-12-2100', co2_scen = 'both')
df_hybrid_proj_ukesm_126_arg, df_prediction_proj_ukesm_126_arg = projections_generation_hybrid(model = 'ukesm', rcp_scenario = 'ssp126', region = "_arg", hybrid_model_full = full_model_hyb2_am2, start_date='01-01-2015', end_date='31-12-2100', co2_scen = 'both')

# GFDL model
df_hybrid_proj_gfdl_585, df_prediction_proj_gfdl_585 = projections_generation_hybrid(model = 'gfdl', rcp_scenario = 'ssp585', region = "_arg", hybrid_model_full = full_model_hyb_arg, start_date='01-01-2015', end_date='31-12-2100', co2_scen = 'both')
df_hybrid_proj_gfdl_126, df_prediction_proj_gfdl_126 = projections_generation_hybrid(model = 'gfdl', rcp_scenario = 'ssp126', region = "_arg", hybrid_model_full = full_model_hyb_arg, start_date='01-01-2015', end_date='31-12-2100', co2_scen = 'both')

# IPSL model
df_hybrid_proj_ipsl_585, df_prediction_proj_ipsl_585 = projections_generation_hybrid(model = 'ipsl', rcp_scenario = 'ssp585', region = "_arg", hybrid_model_full = full_model_hyb_arg, start_date='01-01-2015', end_date='31-12-2100', co2_scen = 'both')
df_hybrid_proj_ipsl_126, df_prediction_proj_ipsl_126 = projections_generation_hybrid(model = 'ipsl', rcp_scenario = 'ssp126', region = "_arg", hybrid_model_full = full_model_hyb_arg, start_date='01-01-2015', end_date='31-12-2100', co2_scen = 'both')


#%% TEST partial dependence plot for out of calibration zone cases - 2088 should be extreme

# load future prodictions
df_predict_fut = df_prediction_proj_ukesm_585_arg.iloc[:,[1]].copy() # Use 2015 case
df_predict_fut = df_predict_fut.rename(columns={'yield-soy-noirr_2015co2':'yield-soy-noirr'})

df_proj_fut = df_hybrid_proj_ukesm_585_arg.rename(columns={'yield-soy-noirr_2015co2':'yield-soy-noirr'})
...
df_hybrid_arg_test = df_hybrid_arg.copy()

# EPIC
plt.plot(df_hybrid_arg_test['yield-soy-noirr'].groupby('time').mean(), label = 'History')
...
# TMX
plt.plot(df_hybrid_arg_test['txm_4'].groupby('time').mean(), label = 'History')
...
# HYBRID
plt.plot(df_predict_hist_arg['Yield'].groupby('time').mean(), label = 'History')
...
for feature in df_proj_fut.columns:
    df_clim_extrapolated = df_proj_fut[feature].where(df_proj_fut[feature] > df_hybrid_arg_test[feature].max()).dropna()
    df_y_extrapolated = df_predict_fut['yield-soy-noirr'].where(df_proj_fut[feature] > df_hybrid_arg_test[feature].max()).dropna()

    plt.scatter(df_hybrid_arg_test[feature], df_predict_hist_arg['Yield'], color = 'k', label = 'History')    
    plt.scatter(df_proj_fut[feature], df_predict_fut['yield-soy-noirr'], alpha = 0.8, label = 'Projection')
    sns.regplot(df_hybrid_arg_test[feature], df_predict_hist_arg['Yield'], color = 'k', label = 'History', scatter = False)    
...
for feature in df_proj_fut.columns:   
    sns.kdeplot(df_hybrid_arg_test[feature],fill=True, alpha = 0.3, label = 'History')
    sns.kdeplot(df_proj_fut[feature],fill=True, alpha = 0.3, label = 'Proj')
    print('hist', np.round(df_hybrid_arg_test[feature].mean(), 3), 'fut', np.round(df_proj_fut[feature].mean(),3))
    plt.legend()
    plt.show()
    
    plt.plot(df_hybrid_arg_test[feature].groupby('time').mean(), label = 'History')
...
sns.kdeplot(df_predict_hist_arg['Yield'], fill=True, alpha = 0.3, label = 'History')
...
# for feature in df_proj_fut.columns:
#     df_clim_extrapolated = df_proj_fut[feature].where(df_proj_fut[feature] < df_hybrid_arg_test[feature].min()).dropna()
#     df_y_extrapolated = df_predict_fut['yield-soy-noirr'].where(df_proj_fut[feature] < df_hybrid_arg_test[feature].min()).dropna()

#     plt.scatter(df_hybrid_arg_test[feature], df_predict_hist_arg['Yield'], color = 'k')    
#     plt.scatter(df_proj_fut[feature], df_predict_fut['yield-soy-noirr'], alpha = 0.8)
#     plt.hlines(df_predict_hist_arg['Yield'].mean(), df_hybrid_arg_test[feature].min(), df_hybrid_arg_test[feature].max(), color = 'k')
...
features_to_plot = ['yield-soy-noirr','txm_2']
fig, ax1 = plt.subplots(1, 1, figsize=(10, 8), dpi=500)
disp1 = PartialDependenceDisplay.from_estimator(full_model_hyb_arg, df_proj_fut, features_to_plot, pd_line_kw={'color':'r'},percentiles=(0.01,0.99), ax = ax1)
disp2 = PartialDependenceDisplay.from_estimator(full_model_hyb_arg, df_hybrid_arg_test, features_to_plot, ax = disp1.axes_,percentiles=(0,1), pd_line_kw={'color':'k'})
...
disp2 = PartialDependenceDisplay.from_estimator(full_model_hyb2_am, df_hybrid_arg_test, features_to_plot, ax = disp1.axes_,percentiles=(0,1), pd_line_kw={'color':'k'})
plt.ylim(0, 2.6)
plt.setp(disp1.deciles_vlines_, visible=False)
plt.setp(disp2.deciles_vlines_, visible=False)
...
fig, ax1 = plt.subplots(1, 1, figsize=(10, 8), dpi=500)
disp1 = PartialDependenceDisplay.from_estimator(full_model_hyb_arg, df_proj_fut, features_to_plot, pd_line_kw={'color':'r'},percentiles=(0.01,0.99), ax = ax1, method = 'brute')
disp2 = PartialDependenceDisplay.from_estimator(full_model_hyb_arg, df_hybrid_arg_test, features_to_plot, ax = disp1.axes_,percentiles=(0,1), pd_line_kw={'color':'k'}, method = 'brute')
plt.ylim(0, 2.6)
plt.setp(disp1.deciles_vlines_, visible=False)
plt.setp(disp2.deciles_vlines_, visible=False)
...
delta.plot()
# test_values = df_hybrid_arg_test.iloc[[0],:-1].copy()
# test_values.iloc[[0],:] = [0,0,0,0,50,50,50]
# test_prdict = full_model_hyb.predict(test_values)
# print(test_prdict)

DS_test = xr.open_mfdataset('monthly_ukesm_ssp585_arg/txm_MON_climpact.ukesm1-0-ll_r1i1p1f2_w5e5_ssp5-ssp5.nc').where(DS_y_epic_arg_det.mean('time') >= -5.0)
...
DS_test2 = DS_test2.where(DS_y_epic_arg_det.mean('time') >= -5.0)


DS_test2['yield-soy-noirr'].sel(time = 438).plot()
plot_2d_map(DS_test2['yield-soy-noirr'].sel(time = 438))
#%%
