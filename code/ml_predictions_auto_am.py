...
# DS_y_epic_proj_ukesm_585_am_test_us = DS_y_epic_proj_ukesm_585_am.where(DS_historical_hybrid_us['Yield'].mean('time') > -10)

# DS_feature_proj_ukesm_585_am = xr.Dataset.from_dataframe(df_feature_proj_ukesm_585_am)
# DS_feature_proj_ukesm_585_am = rearrange_latlot(DS_feature_proj_ukesm_585_am)
# DS_feature_proj_ukesm_585_am_test_us = DS_feature_proj_ukesm_585_am.where(DS_historical_hybrid_us['Yield'].mean('time') > -10)
# df_feature_proj_ukesm_585_am_test_us = DS_feature_proj_ukesm_585_am_test_us.to_dataframe()



# df_input_hybrid_fut_ukesm_585_am, df_prediction_proj_ukesm_585_am = hybrid_predictions_future(DS_y_epic_proj_ukesm_585_am_test_us, df_feature_proj_ukesm_585_am_test_us, model = 'ukesm', rcp_scenario = 'ssp585', region = "_am", hybrid_model_full = full_model_hyb2_am_rf, co2_scen = co2_scen)
...
# #%% FUNCTION TO GENERATE PROJECTIONS BASED ON RANDOM FOREST
# def past_generation_hybrid(region, hybrid_model_full, start_date, end_date, three_models = False, sim_round = '\Gen_Assem'):
#     if region == '_am' or 'am':  

#         #COMBINE THE THREE REGIONS WITH SOUTH AMERICA 12 MONTHS SHIFTED
#         DS_clim_ext_projections_us = xr.open_mfdataset('../../paper_hybrid_agri/data/climpact-master/climpact-master/www/output_historical_us/monthly_data/*.nc').sel(time=slice(start_date, end_date))

#         # Shift br data one year ahead
#         DS_clim_ext_projections_br =xr.open_mfdataset('../../paper_hybrid_agri/data/climpact-master/climpact-master/www/output_gswp3/monthly_data/*.nc').sel(time=slice(start_date, end_date))

#         DS_clim_ext_projections_br = DS_clim_ext_projections_br.copy().shift(time = 12) # SHIFT EPIC BR ONE YEAR FORWARD
       
#         # Shift ARG data one year ahead
#         DS_clim_ext_projections_arg = xr.open_mfdataset('../../paper_hybrid_agri/data/climpact-master/climpact-master/www/output_historical_arg/monthly_data/*.nc').sel(time=slice(start_date, end_date))
#         DS_clim_ext_projections_arg = DS_clim_ext_projections_arg.copy().shift(time = 12) # SHIFT EPIC BR ONE YEAR FORWARD
...
#     # Choose historical mean so there is no mean deviation between the historical and future timelines
#     DS_feature_proj_6mon_am_det = detrend_dataset(DS_feature_proj_6mon_am, deg = 'free', mean_data = DS_feature_season_6mon_am_det.sel(time = slice(2000,2015))) 
...
#             DS_y_epic_proj = xr.open_dataset("epic-iiasa_gswp3-w5e5_obsclim_2015soc_default_yield-soy-noirr_global_annual_1901_2016.n", decode_times=False)
#             DS_y_epic_proj = convert_timeunit(DS_y_epic_proj)
...
#         else:
#             DS_y_epic_proj = xr.open_dataset("epic-iiasa_past_w5e5_yield-soy-noirr_global_annual_1900_1970.nc", decode_times=False)
#             DS_y_epic_proj_am = DS_y_epic_proj.where(DS_y_epic_am_det_regul.mean('time') >= -5.0 )
                    
#         return DS_y_epic_proj_am, DS_feature_proj_6mon_am, df_feature_proj_6mon
# # =============================================================================    
...
# # Prediction past-pre training

# DS_y_epic_proj_ukesm_585_am, DS_feature_proj_ukesm_585_am, df_feature_proj_ukesm_585_am = past_generation_hybrid(region = "_am", hybrid_model_full = model_to_be_used, start_date='1900-12-12', end_date='1972-12-12')
